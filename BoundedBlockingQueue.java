...
/**
 * A high performance, thread-safe FIFO Bounded Blocking Queue designed for back pressure management and 
 * resource governance in high-scale ingestion pipelines. This queue forces producers to block when the 
...
public class BoundedBlockingQueue<T> {
    ...
    // Do we need a fairness policy for the lock? If multiple producers are waiting, should the one who has been waiting the
    // longest get the next slot? ReentrantLock(true) ensures no thread starves, but it has a massive thoughput penalty because
    // it forces more complex thread scheduling. In high scale data platforms, we usually prefer unfair locks for max performance
    private final int capacity;    private final ReentrantLock lock = new ReentrantLock();

    // Since we have multiple producers and consumers, we must use signal() and notify() more carefully. If we have many consumers, we need to ensure our
    ...
    private final Condition notEmpty = lock.newCondition(); // The consumer is waiting for the state to become not empty.

    public BoundedBlockingQueue(int capacity) {
        this.capacity = capacity;
        // ArrayDeque is backed by a contiguous array, while LinkedList is composed of scattered node objects.
        // Since array object references are stored next to each other, the CPU can prefetch them into its cache, causing fewer cache misses since
        // navigating through a LinkedList requires following pointers to random memory addresses in the heap.
    ...
    public void enqueue(T item) throws InterruptedException {
        ...
        lock.lock();
        try {
            // Why a while loop? The thread wakes up, goes back to the top of the loop, checks the condition again, sees it's full
            // and immediately goes back to sleep
            while (queue.size() == this.capacity) {
                System.out.println("Producer thread is waiting inside the conditional loop.");

                // await is infinite so the thread is waiting forever. While this is a great place to start, its not ideal for production code. An alternative
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // Force main thread to sleep so that the consumer thread can enter the while loop
